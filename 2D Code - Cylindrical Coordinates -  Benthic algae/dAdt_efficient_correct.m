function [output] = dAdt_efficient_correct(t,Y,p)
...
%         %dAdt_temp(i,j) = integral_A_1(i,j,p,A) +
%         %dAdt_temp(i,j) = integral_A_1(i,j,p,A) + integral_A_2(i,j,p,A);
%         %dAdt_temp(i,j) = integral_A_3(i,j,p,A);
%          dAdt_temp(i,j) = integral_A_1(i,j,p,A) + integral_A_2(i,j,p,A) + integral_A_3(i,j,p,A) + integral_A_4(i,j,p,A);
%          %dRdt_temp(i,j) = integral_Rd_1(i,j,p,Rd) + integral_Rd_2(i,j,p,Rd) + integral_Rd_3(i,j,p,Rd);
%          dRdt_temp(i,j) = integral_Rd_4(i,j,p,Rd,Rs,B);
%        % temp = 1;
%     end
% end
%
...
for j = 1:p.Xn-1 % xi
    ...
    nutrient_limited_growth = 1/p.q_benth*Rs(j)*p.r +  p.Gmax_benth*B(j)*(Rd(i,j)/(Rd(i,j)+p.M_benth)) + ...
    ...
    if(efficient)
        ...
        nutrient_limited_flux = Rs(j)*p.r +  p.q_benth*p.Gmax_benth*(Rd(i,j)/(Rd(i,j)+p.M_benth))* B(j) ...
            + p.q_benth*p.benth_recycling*p.lbg_benth*B(j); % [mgP/ (m^2 day)]
