...
p.benth_recycling = 0.1; % Governs the portion of respired nutrients that are released as dissolved nutrients.
...
%[t,Y_t] = ode15s( @(t,Y) dAdt_efficient_correct_V2(t,Y,p) , [0, inf] , y0 , ode_opts); % diffusion works in this version!
...
B = Y_t(end,2*(p.Xn-1)*(p.Zn-1) + (p.Xn-1)+1: end);
