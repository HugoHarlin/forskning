...
parfor p = 1:3
    %p = 1;
    for depth_var =  1  % looping over thermocline depth
        for resus_index = 2 :4
            %depth_var = 1;
            % for x_res_index = [0,1]
            ...
            pp.Xn   = 31;  % Number of grid-points (width)
            pp.Zn   = 31;  % Number of grid-points (depth)
            ...
            if(~pp.stratified)
                ...
                diff_max_x = diff_vec(3); % horizontal diffusion coefficient at the surface
                ...
                diff_min_z = diff_max_x; % diff_max_x; % vertical diffusion coefficient at the bottom
                diff_max_z = diff_max_x; % diff_max_x;
            ...
            if(~pp.periodic_mixing) % no periodic mixing
                ...
                while(t_var <  pp.max_sim_time)
                    [t,Y_t] = ode15s( @(t,Y) rhs_function_V6(t,Y,pp), [ t_var,  t_var + pp.mixing_frequency] , y0 , ode_opts);
            ...
            if(1)
                ...
                file_name = file_name + "_resus_rate_" + strrep(num2str(pp.resus),'.','_') ;
