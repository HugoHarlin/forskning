...
parfor p =  1:13
    for manual_resus_indx = 1 % :3
        ...
        for kbg_index = 2 %:2 % :3
           ...
           width_vec = [20,50,100,500];
            pp.W    =  width_vec(1);  % Lake radius [m]
            ...
            pp.seasonality_light = 0; % seasonally varying light (sinusoidal)
            pp.maxLight = 400; % maximum light intesity for seasonally varying light [micro-mol photons m^-2 s^-1]
            pp.minLight = 20; % minimum light intesity for seasonally varying light [micro-mol photons m^-2 s^-1]
            ...
            if(~pp.stratified)
                ...
                diff_min_x =  diff_vec(diff_index); % horizontal diffusion coefficient at the bottom
                diff_min_z =  diff_vec(4); % diff_max_x; % vertical diffusion coefficient at the bottom
                diff_max_z = diff_min_z; % diff_max_x;
                %diff_min_z = diff_max_z; % vertical diffusion coefficient at the bottom
                
                dx = diff_max_z.*ones(pp.Zn-1,pp.Xn-1);
                ...
                for x = 1:pp.Xn-1
                    for z = 1:pp.Zn-1
                        ...
                        if(pp.Z_vol(z,x) < pp.thermocline_depth)
                            pp.dx(z,x) =  pp.diff_above_thermocline; % *(pp.Zn-z)/pp.Zn;
            ...
            if(pp.constant_resuspension)
                resus_vec = [0, 0.01, 0.05, 0.1 0.2]; % 0.2 is used as default value
                resus_index = 3;
            ...
            nutr_tot_start = (pi*pp.W^2)*nutr_area; % total amount of starting nutrients in lake [mgP/m^3]
