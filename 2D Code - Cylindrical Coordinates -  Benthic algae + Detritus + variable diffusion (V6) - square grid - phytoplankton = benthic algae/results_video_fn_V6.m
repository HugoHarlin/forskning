...
function [] = results_video_fn_V6(t,Y_t,p)
...
for vid_indx=(length(t)-365):31:length(t)
    clf(f1);
    
    %% Extracting results
    ntot_0 = p.ntot_0;
    
    A  = Y_t(vid_indx,1:sum(1:p.Xn-1)); % phytoplankton
    Rd = Y_t(vid_indx,sum(1:p.Xn-1)+1:2*sum(1:p.Xn-1)); % dissolved nutrients
    D  = Y_t(vid_indx,2*sum(1:p.Xn-1) +1 : 3*sum(1:p.Xn-1)); % ditritus
    Rs = Y_t(vid_indx,3*sum(1:p.Xn-1)+1 : 3*sum(1:p.Xn-1)+ p.Xn-1); % sedimented nutrients
    B  = Y_t(vid_indx, 3*sum(1:p.Xn-1)+ p.Xn :  3*sum(1:p.Xn-1)+ 2*(p.Xn-1)); % benthic algae
    ...
    if(p.seasonality_light)
        ...
        p.I0 = (p.maxLight-p.minLight)/2*sin(2*pi*(90+mod(t(vid_indx),365))/365) + (p.minLight + p.maxLight)/2;
    end
    ...
    if(true)
        ...
        if(true)
            ...
            title_str = title_str + "resolution: " + num2str(p.Xn) + " kbg: " + num2str(p.kbg) + ""; %  + " alpha: " + num2str(p.alpha)
            ...
            %% setting therm depth for seasonal variation = 1
            
            if(p.stratified)
                if(p.seasonality)
                    if(p.seasonality_thermoC) % varying depth of the thermocline
                        p.thermocline_depth = (p.maxTherm -p.minTherm)/2*sin(2*pi*(90+mod(t(vid_indx),365))/365) + (p.minTherm + p.maxTherm )/2;
                    end
                end
            end
            ...
            if(lim_switch_index ~= 0)
                ...
                for i=1:length(lim_switch_index)
                    ...
                    hold on
                    yyaxis left
                    plot(query_points(start_index:lim_switch_index(i)), benth_interp(start_index:lim_switch_index(i)),color, 'LineWidth', 1.5);
                ...
                yyaxis left
                plot(query_points(lim_switch_index(end):end), benth_interp(lim_switch_index(end):end),'-', 'color', color, 'LineWidth', 1.5);
            else
                ...
                yyaxis left
                plot(query_points, benth_interp,color, 'LineWidth', 1.5);
            end
