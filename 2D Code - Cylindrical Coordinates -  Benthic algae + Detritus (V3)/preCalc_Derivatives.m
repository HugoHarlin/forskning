...
%             ( dZ_dEta(p,i-0.5,j) *0.25*(A(i-1,j+1) + 2*A(i,j+1) + A(i+1,j+1) - A(i+1,j) - 2*A(i,j) - A(i-1,j)) ...
%             - dZ_dXi(p,i-0.5,j) * 0.25 *( A(i+1,j) + A(i+1,j+1) - A(i-1,j) - A(i-1,j+1) ));
        

        % Integral 2 - alternative derivative approximation
%         dAdt(i,j) = dAdt(i,j) + (p.dx * dZ_dXi(p,i-1,j-0.5)/dX_dXi(p,i-1,j-0.5)* ...
%             0.25*(A(i-1,j+1) + A(i,j+1) - A(i-1,j-1) - A(i,j-1)) - 1/( dX_dXi(p,i-1,j-0.5) ...
%         *dZ_dEta(p,i-1,j-0.5))*0.25*(A(i,j-1) + 2*A(i,j) + A(i,j+1) - A(i-1,j+1) - 2*A(i-1,j) - A(i-1,j-1) )* ...
%             (p.dx*dZ_dXi(p,i-1,j-0.5)^2 + p.dZ*dX_dXi(p,i-1,j-0.5)^2));
        

                % Integral 3 - alternative derivative approximation
%         dAdt(i,j) = dAdt(i,j) + sqrt( dX_dEta(p,i-0.5,j-1)^2 + dZ_dEta(p,i-0.5,j-1)^2) / abs( dX_dXi(p,i-0.5,j-1)* ...
%             dZ_dEta(p,i-0.5,j-1)) * p.dx*( dZ_dXi(p,i-0.5,j-1)*0.25*(A(i+1,j-1)+A(i+1,j)-A(i-1,j-1)-A(i-1,j)) - ...
%             dZ_dEta(p,i-0.5,j-1) * 0.25*(A(i+1,j) + 2*A(i,j) + A(i-1,j) - A(i-1,j-1) - 2*A(i,j-1) - A(i+1,j-1) ));

        % Integral 4 - alternative derivative approximation
%         dAdt(i,j) = dAdt(i,j) + 1/abs(dX_dXi(p,i,j-0.5) * dZ_dEta(p,i,j-0.5))*p.dZ*dX_dXi(p,i,j-0.5)^2 * ...
%             0.25*(A(i+1,j-1) + 2*A(i+1,j) + A(i+1,j+1) - A(i,j+1) -2*A(i,j) - A(i,j-1) ) + p.dx*dZ_dXi(p,i,j-0.5)^2* ...
%             0.25*(A(i+1,j-1) + 2*A(i+1,j) + A(i+1,j+1) - A(i,j+1) -2*A(i,j) - A(i,j-1) ) -  ...
%             p.dx*dZ_dXi(p,i,j-0.5)*dZ_dEta(p,i,j-0.5)*0.25*(A(i,j+1) + A(i+1,j+1) - A(i,j-1) - A(i+1,j-1));        
