...
p = struct;
p.dx = 1; % Radial Turbulent-diffusion coefficient [m^2 day^-1]
p.dy = 100; % Vertical Turbulent-diffusion coefficient [m^2 day^-1]
p.I0 = 1400; %300; % Light intensity at the surface [micro-mol photons m^-2 s^-1]
...
p.lbg = 0; %0.2; % Specific algal maintenance respiration losses [day^-1]
...
p.H = 1; %120.0; % Half-saturation constant of light-dependent algal production [micro-mol photons m^-2 s^-1]
p.q = 0.0244; % Algal nutrient quota, Redfield ratio
p.r = 0.02; % Specific mineralization rate of sedimented nutrients [day^-1]
p.v =0; %0.25; % Algal sinking velocity [m day^-1]
...
p.Lmin = 100; % Minimum lake depth (depth at land-water interface) [m]
p.Lmax = 100; % Maximum lake depth [m]
p.W = 	100; % Lake width [m]
...
A0 = 1*ones(p.Yn-1, p.Xn-1); % Initial Algal carbon density [mg C m^-3]
A0(15,15) = 2;
Rd0 = 0.00*ones(p.Yn-1, p.Xn-1); % initial concentration of dissolved nutrients [mg P m^-3]
