function [dX_dXi_preCalc, dX_dEta_preCalc, dY_dXi_preCalc, dY_dEta_preCalc] = preCalc_Derivatives(p)
...
dX_dXi_preCalc = 0*ones(p.Yn-1, p.Xn-1,4);
dX_dEta_preCalc = 0*ones(p.Yn-1, p.Xn-1,4);
dY_dXi_preCalc = 0*ones(p.Yn-1, p.Xn-1,4);
dY_dEta_preCalc = 0*ones(p.Yn-1, p.Xn-1,4);
...
for i = 1:p.Yn-1 % eta
    for j = 1:p.Xn-1 % xi
        ...
        dY_dXi_preCalc(i,j,1)  = dY_dXi(p,i-0.5,j);
        dY_dEta_preCalc(i,j,1) = dY_dEta(p,i-0.5,j);
        
        dX_dXi_preCalc(i,j,2)  = dX_dXi(p,i-1,j-0.5);
        dX_dEta_preCalc(i,j,2) = dX_dEta(p,i-1,j-0.5);
        dY_dXi_preCalc(i,j,2)  = dY_dXi(p,i-1,j-0.5);
        dY_dEta_preCalc(i,j,2) = dY_dEta(p,i-1,j-0.5);
        
        dX_dXi_preCalc(i,j,3)  = dX_dXi(p,i-0.5,j-1);
        dX_dEta_preCalc(i,j,3) = dX_dEta(p,i-0.5,j-1);
        dY_dXi_preCalc(i,j,3)  = dY_dXi(p,i-0.5,j-1);
        dY_dEta_preCalc(i,j,3) = dY_dEta(p,i-0.5,j-1);        
        
        dX_dXi_preCalc(i,j,4)  = dX_dXi(p,i,j-0.5);
        dX_dEta_preCalc(i,j,4) = dX_dEta(p,i,j-0.5);
        dY_dXi_preCalc(i,j,4)  = dY_dXi(p,i,j-0.5);
        dY_dEta_preCalc(i,j,4) = dY_dEta(p,i,j-0.5);            
