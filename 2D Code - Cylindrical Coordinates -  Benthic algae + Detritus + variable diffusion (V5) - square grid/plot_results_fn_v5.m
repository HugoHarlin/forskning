...
A = Y_t(end,1:sum(1:p.Xn-1)); % phytoplankton [mgP/m^3]
Rd = Y_t(end,sum(1:p.Xn-1)+1:2*sum(1:p.Xn-1)); % dissolved nutrients [mgP/m^3]
D = Y_t(end,2*sum(1:p.Xn-1) +1 : 3*sum(1:p.Xn-1)); % ditritus [mgP/m^3]
Rs = Y_t(end,3*sum(1:p.Xn-1)+1 : 3*sum(1:p.Xn-1)+ p.Xn-1); % sedimented nutrients [mgP/m^2]
B = Y_t(end, 3*sum(1:p.Xn-1)+ p.Xn :  3*sum(1:p.Xn-1)+ 2*(p.Xn-1)); % benthic algae [mgP/m^2s]
...
if(true)
    ...
    if(true)
        ...
        trisurf([1 2 3], [0 p.W p.W], [p.Lmax p.Lmax 0],(1+max(max(p.q.*A_interp))).*[1 1 1], 'edgecolor','none', 'FaceColor','white')
        hold on
        
        % if stratified, the thermocline is plotted as a gray horizontal bar
        if(p.stratified)
            therm =  surf([0 0; p.W p.W],[p.thermocline_depth p.thermocline_depth + p.thermocline_thickness; p.thermocline_depth p.thermocline_depth + p.thermocline_thickness], (1+max(max(p.q.*A_interp))).*[1 1; 1 1]);
       ...
       % caxis([0 inf]);
        ...
        % the diagonal.
        trisurf([1 2 3], [0 p.W p.W], [p.Lmax p.Lmax 0],(1+max(max(Rd_interp))).*[1 1 1], 'edgecolor','none', 'FaceColor','white') % plotting the diagonal elements
        hold on
        
        % if stratified, the thermocline is plotted as a gray horizontal bar
        if(p.stratified)
            therm =  surf([0 0; p.W p.W],[p.thermocline_depth p.thermocline_depth + p.thermocline_thickness; p.thermocline_depth p.thermocline_depth + p.thermocline_thickness], (2+max(max(Rd_interp))).*[1 1; 1 1]);
            therm.EdgeColor = 'none';
            therm.FaceAlpha  = 0.1;
            therm.FaceColor = 'black';
        end
        ...
        trisurf([1 2 3], [0 p.W p.W], [p.Lmax p.Lmax 0],(1+max(max(D_interp))).*[1 1 1], 'edgecolor','none', 'FaceColor','white') % plotting the diagonal elements
        hold on
        
        % if stratified, the thermocline is plotted as a gray horizontal bar
        if(p.stratified)
            therm =  surf([0 0; p.W p.W],[p.thermocline_depth p.thermocline_depth + p.thermocline_thickness; p.thermocline_depth p.thermocline_depth + p.thermocline_thickness], (2+max(max(D_interp))).*[1 1; 1 1]);
            therm.EdgeColor = 'none';
            therm.FaceAlpha  = 0.1;
            therm.FaceColor = 'black';
        end
        ...
        benthic_limitation =   interp1(sed_points, benthic_limitation, query_points,'pchip','extrap');
        
        lim_switch_index = 0;
        index = 1;
        % not the prettiest but get the job done...
        for i=1:length(benthic_limitation)-1
            if(  sign(benthic_limitation(i)) ~= sign(benthic_limitation(i+1)) )
        ...
        if(lim_switch_index ~= 0)
            ...
            for i=1:length(lim_switch_index)
                if(sign(benthic_limitation(lim_switch_index(i)-1)) < 0)
            ...
            plot(query_points(lim_switch_index(end):end), benth_interp(lim_switch_index(end):end),color);
        else
            
            if(benthic_limitation(end) == -1)
                color = 'red';
            else
                color = 'blue';
            end
        ...
        hold on
        if(p.stratified)
            therm =  surf([0 0; p.W p.W],[p.thermocline_depth p.thermocline_depth + p.thermocline_thickness; p.thermocline_depth p.thermocline_depth + p.thermocline_thickness], [2 2; 2 2]);
            therm.EdgeColor = 'none';
            therm.FaceAlpha  = 0.1;
            therm.FaceColor = 'black';
        end
...
%        pelagic_primary_prod = phytoplankton_primary_prod.*p.volumes_cyl;
 ...
 %       str  = append(str, 'benthic contribution to total primary production:            ', num2str(sum(benth_primary_prod)/total_lake_primary_prod), '\n');
  ...
  %      str  = append(str, 'average total primary production [mgC/(day m^2)]:        ', num2str(total_lake_primary_prod/sum(p.Area_bottom_cyl)), '\n');
    ...
    if(p.W ~= 20)
        fig_name = fig_name + "_width_" + strrep(num2str(p.W),'.','_');
    end
