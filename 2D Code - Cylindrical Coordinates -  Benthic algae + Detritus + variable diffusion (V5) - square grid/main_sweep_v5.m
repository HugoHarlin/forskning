...
for p = 1 %:3
    %p = 1;
    for depth_var =  2 % looping over thermocline depth
        %depth_var = 1;
        % for x_res_index = [0,1]
       ...
       % pp.Zn   = res_vec(p);  % Number of grid-points (depth)
        ...
        kgb_vec = [0.2 0.6 2.0 3.0 5.0 8.0];
        ...
        if(~pp.stratified)
            ...
            diff_min_z = 10; % diff_max_x; % vertical diffusion coefficient at the bottom
            diff_max_z = 10; % diff_max_x;
