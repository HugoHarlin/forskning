...
function [] = results_video_fn_v5(t,Y_t,p)
...
for i=1:30:length(t)
    ...
    A = Y_t(i,1:sum(1:p.Xn-1)); % phytoplankton
    Rd = Y_t(i,sum(1:p.Xn-1)+1:2*sum(1:p.Xn-1)); % dissolved nutrients
    D = Y_t(i,2*sum(1:p.Xn-1) +1 : 3*sum(1:p.Xn-1)); % ditritus
    Rs = Y_t(i,3*sum(1:p.Xn-1)+1 : 3*sum(1:p.Xn-1)+ p.Xn-1); % sedimented nutrients
    B = Y_t(i, 3*sum(1:p.Xn-1)+ p.Xn :  3*sum(1:p.Xn-1)+ 2*(p.Xn-1)); % benthic algae
    
    
    %Calculating nutrient content at t = Tend
    n_algae_t     = p.volumes_cyl.*A'*p.q;
    n_dissolved_t = p.volumes_cyl.*Rd';
    n_detritus_t  = p.volumes_cyl.*D';
    n_sediment_t  = Rs.*p.Area_bottom_cyl;
    n_benthic_t   = B.*p.q_benth.*p.Area_bottom_cyl;
    ntot_end        = sum(sum(n_algae_t)) + sum(sum(n_dissolved_t)) + sum(sum(n_detritus_t)) + sum(n_sediment_t) + sum(n_benthic_t);
    ...
    %% plots of all concentrations in one figure using griddata
        ...
        %surf(p.X_vol, p.Z_vol, Rd_matrix_temp,  'edgecolor','none');
        ...
        %% Detritus
        nexttile
        %surf(p.X_vol, p.Z_vol, D_matrix_temp,  'edgecolor','none');
        ...
        delete(t);
        dim = [.11 .1 .2 .2];
        perc_algae = sum(sum(n_algae_t))/ntot_end;
        perc_dissolved = sum(sum(n_dissolved_t))/ntot_end;
        perc_detritus = sum(sum(n_detritus_t))/ntot_end;
        perc_sediment = sum(n_sediment_t)/ntot_end;
        perc_benthic = sum(n_benthic_t)/ntot_end;
