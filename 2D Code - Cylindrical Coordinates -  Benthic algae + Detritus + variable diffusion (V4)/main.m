...
p.lbg_A =  0.1; % Specific algal maintenance respiration losses [day^-1]
p.Ad = 0; %0.02; % algal death rate [day^-1]
...
p.resus = 0; % resuspension rate of the detritus from the sediment. [day^-1]
p.resus_Max =  0; %0.02; % Maximum resuspension rate of the detritus in the sediment [day^-1]
...
p.alpha = 1.2; % Exponent governing the slope of the lake bottom
...
ode_opts = odeset( 'abstol' , 1e-9 , 'reltol' , 1e-9 , 'Events', @(t,y) eventfun_V4(t,y,p)); % , 'NonNegative',(1:length(y0)));
%[t,Y_t] = ode15s( @(t,Y) dAdt_efficient_correct_V4_detritus(t,Y,p) , [0, inf] , y0 , ode_opts);
...
simTime = toc;
%file_name = "2D_benthic_results_V4_dx_" + num2str(p.dx) + "_dz_" + num2str(p.dz) + "_Xn_" + num2str(p.Xn) + "_Zn_" + num2str(p.Zn);
%save(file_name);
