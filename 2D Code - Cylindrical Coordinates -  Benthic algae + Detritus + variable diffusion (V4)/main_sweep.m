...
parfor p = 1:4
    ...
    for diff_index = 1:3
        ...
        kgb_vec = [0,0.02,0.4, 0.8];
        pp.kbg = kgb_vec(p); %0.2;   % Background light-attenuation coefficient [m^-1]
        ...
        pp.Xn   = 26;  % Number of grid-points (width)
        pp.Zn   = 26;  % Number of grid-points (depth)
        pp.Lmin = 0.1; % Minimum lake depth (depth at land-water interface) [m]
        pp.Lmax = 20;  % Maximum lake depth [m]
        pp.W    = 20;  % Lake radius [m]
        alpha_vec = [0.5, 1, 1.5]; % slopes being looped over.
        pp.alpha = alpha_vec(diff_index);  % Exponent governing the slope of the lake bottom
        ...
        for diff_max_x = [1,10,100,1000]
            for diff_max_z = [1,10,100,1000]
                ...
                simTime = toc;
                alpha_str = ["0_5","1_0","1_5"];
                kbg_str = ["0","0_02","0_4", "0_8"];
                file_name = "2D_benthic_results_V4_dx_" + num2str(diff_max_x) + "_dz_" + num2str(diff_max_z) + "_Xn_" + num2str(pp.Xn) + "_Zn_" + num2str(pp.Zn) + "_alpha_" + alpha_str(diff_index) + "_kgb_" +  kbg_str(p);
