...
for diff_max_x = [1,10,100,1000]
    for diff_max_z = [1,10,100,1000]
        ...
        ode_opts = odeset( 'abstol' , 1e-7 , 'reltol' , 1e-7 , 'Events', @(t,y) eventfun_V4(t,y,p), 'NonNegative',(1:length(y0)));
        ...
        simTime = toc;
        file_name = "2D_benthic_results_V4_dx_" + num2str(diff_max_x) + "_dz_" + num2str(diff_max_z) + "_Xn_" + num2str(p.Xn) + "_Zn_" + num2str(p.Zn) + "_alpha_" + num2str(p.alpha);
        save(file_name);
